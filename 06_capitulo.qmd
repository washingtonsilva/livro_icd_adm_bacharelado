...
rotineiras, o que aumenta significativamete sua produtividade.
...
Como usaremos Git e GitHub, é importante configurar o terminal do RStudio 
para abrir com o Git Bash, permitindo executar comandos do Git sem sair do RStudio.
...
- Se a configuração tiver sido feita corretamente (ou se o RStudio já tiver
  detectado o Git Bash), o terminal exibirá um prompt parecido com:

  `usuario@PC MINGW64 ...`

  *(a presença de “MINGW64” é um indício forte de que você está no Git Bash).*

- Para confirmar, digite o comando `pwd` e pressione **Enter**.
  ...
  - Esse caminho deve corresponder à pasta do seu projeto (a pasta onde está o
    arquivo `.Rproj`) que estiver aberto no RStudio.
...
Em muitos computadores com **Git for Windows** instalado (o pacote que inclui o *Git Bash*),
o RStudio consegue **detectar automaticamente** o *Git Bash* e defini-lo como o terminal 
padrão do RStudio, **sem que você precise configurar nada**.  

Ainda assim, é recomendável verificar esta opção em *Tools* → *Global Options* → *Terminal*,
porque em alguns computadores o “terminal padrão” pode ser outro (por exemplo, *PowerShell*).

Se não estiver como *Git Bash*, basta selecionar manualmente, como indicado abaixo.
:::
