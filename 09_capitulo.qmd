...
Este capítulo aborda o controle de versão de documentos 
computacionais usando o sistema Git e a plataforma GitHub.
...
testar outros comandos úteis para o dia a dia, como os 
comandos `git fetch`, `git diff` e `git restore`.
...
## Introdução a *Branches* (Ramificações)
...
- **Analogia**: Se o seu projeto fosse um livro, o branch principal (`main`) seria a versão oficial. Um branch seria como fazer uma fotocópia do livro para testar uma nova estrutura de capítulos - você pode experimentar sem estragar o original.
...
**Exemplo:**
```bash
git branch experimento-analise
```
...
**Exemplo:**
```bash
git checkout experimento-analise
```
...
**Exemplo:**
```bash
git checkout -b revisao-metodologia
```
...
### Workflow Básico com Branches

Exemplo Prático: Experimentando uma Nova Análise

Imagine que você quer testar uma abordagem diferente de análise 
de dados, mas não quer perder seu trabalho atual:
...
git checkout -b teste-nova-analise
```

**Passo 2: Trabalhar no branch experimental**
...
git add .
git commit -m "Adiciona análise experimental com método X"
```

**Passo 3: Comparar com o trabalho original**
...
# Seus arquivos mostram a versão experimental
```
...
git branch -d teste-nova-analise
```

Se não gostou do experimento:
```bash
# Voltar para main
git checkout main
...
**Dica**: Use `-d` para deletar um branch que já foi mergeado, e `-D` para forçar a deleção de um branch não mergeado.



### Branches em Colaboração

::: {.callout-note icon="false"}
## Usando Branches para Colaboração Avançada
...
git push origin capitulo4-resultados
```

**2. Orientador trabalhando em revisões em branch separado:**
...
**3. Incorporar o trabalho ao projeto principal:**
...
git push origin --delete capitulo4-resultados
```
...
- Edite o arquivo para manter o texto desejado
- Remova as marcações `<<<<<<<`, `=======`, e `>>>>>>>`
- Salve o arquivo

**3. Finalizar o merge:**

```bash
git add capitulo2.qmd
git commit -m "Resolve conflito entre main e novo-capitulo"
```
...
A seguir, são elencadas recomendações para o uso 
de Branches:
...
git checkout -b temporario
```

**2. Branches de vida curta:**
...
- Comece usando apenas o branch `main` até se sentir confortável
- Experimente branches para tarefas pequenas e bem definidas
